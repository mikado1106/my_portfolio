...
function OrgCard({ role, name, period, items, tags }: {
  ...
  return (
    <div className="h-full p-6 rounded-2xl bg-[var(--bg-card)] border border-[var(--border)] hover:border-[var(--border-hover)] transition-colors">
      <div className="flex items-start justify-between gap-2 mb-5">
        <div>
          <h3 className="text-base font-bold text-[var(--text)]">{role}</h3>
      ...
      <ul className="space-y-2 mb-5">
        {items.map((item, i) => (
          <li key={i} className="text-sm text-[var(--text-secondary)] flex items-start gap-3">
            <span className="text-[var(--green)] mt-0.5 text-xs" aria-hidden="true">▸</span>
